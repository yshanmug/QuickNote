        ...
        TextField(
            ...
            placeholder = {
                Text(
                    "Title..",
                    fontSize = 20.sp,
                    color = if (isSystemInDarkTheme()) MaterialTheme.colorScheme.onTertiary.copy(
                        alpha = 0.6f
                    )
        ...
        if (isToDoList) {
            ...
            LazyColumn(modifier = Modifier.fillMaxSize()) {
                items(combinedItems, key = { it.id }) { item ->
                    DynamicRow(toDoItem = item,
                        onDelete = {
                            notesViewModel.deletedToDoItem(item.id)
                        },
                        onValueChange = { toDoItem ->
                            notesViewModel.updateToDoItem(toDoItem)
                ...
                item {
                    Row(
                        Modifier
                            ...
                            .clickable {
                                notesViewModel.addToDoItem(
                                    noteId = 0, toDoText = "", isChecked = false
                                )
                                focusRequester.add(FocusRequester())
                            },
                        ...
                        Icon(
                            Icons.Filled.Add,
                            tint = if (isSystemInDarkTheme()) MaterialTheme.colorScheme.onTertiary.copy(
                                alpha = 0.6f
                            )
                            else MaterialTheme.colorScheme.onTertiary,
...
fun DynamicRow(
    ...
    Row(
        ...
        Checkbox(
            ...
            }, modifier = Modifier.padding(top = 4.dp), colors = CheckboxDefaults.colors(
...

        {
            Icon(
                Icons.Filled.Clear,
                tint = if (isSystemInDarkTheme()) MaterialTheme.colorScheme.onTertiary.copy(alpha = 0.6f)
                else MaterialTheme.colorScheme.onTertiary,
